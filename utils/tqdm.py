...
from typing import Iterable,Iterator,Generic,Optional,TypeVar
...
class tqdm(Generic[T]):
    ...
    def update(
        ...
        def SI(x): 
            return (f"{x/1000**int(g:=math.log(x,1000)):.{int(3-3*math.fmod(g,1))}f}"[:4].rstrip('.')+' kMGTPEZY'[int(g)].strip()) if x else '0.00'
